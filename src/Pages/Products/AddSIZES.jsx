...
const AddSIZES = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (editId === "") {
            postData('/api/product/productSize/create', { name: name })
                .then((res) => {
                    if (res?.error === false) {
    ...
    const deleteItems = (id) => {
        deleteData(`/api/product/productSize/${id}`).then((res) => {
            getData();
            context.openAlertBox("success", "Size deleted");
        })
    }
    const editItem = (id) => {
            ...
            <div className="card my-4 pt-5 pb-5 shadow-md sm:rounded-lg bg-white w-full md:w-[65%] px-2">
                <form className='form py-3 p-4 md:p-6' onSubmit={handleSubmit}>
                    <div className='col mb-4'>
                        <h3 className='text-[14px] md:text-[16px] font-[500] mb-1 text-black uppercase tracking-wider'>PRODUCT SIZE</h3>
