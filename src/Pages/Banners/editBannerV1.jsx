...
export const EditBannerV1 = () => {
    ...
    const handleSubmit = (e) => {
        ...
        //     return false;
        // }
        editData(`/api/bannerV1/${context?.isOpenFullScreenPanel?.id}`, formFields)
            .then((res) => {
                setTimeout(() => {
                    setIsLoading(false);
        ...
        <section className='p-2 md:p-5 bg-gray-50'>
            <form className='form py-3 p-4 md:p-8 bg-white shadow-sm rounded-md' onSubmit={handleSubmit}>
                <div className="scroll max-h-[70vh] md:max-h-[72vh] overflow-y-auto pr-2 md:pr-4 pt-4">
                    ...
                    <br />
                    <h3 className="text-[16px] font-[500] mb-3 text-black border-b pb-2">Image Management</h3>

                    {/* Systematic Image Preview Grid */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4">
                        {previews?.length !== 0 && previews?.map((image, index) => (
                            <div className="uploadBoxWrapper relative" key={index}>
                                <span className="absolute w-[22px] h-[22px] rounded-full bg-red-700 -top-2 -right-2 flex items-center justify-center z-50 cursor-pointer shadow-md transition-transform hover:scale-110"
                ...
                {/* Systematic Footer Actions */}
                <div className="mt-8 w-full sm:w-[280px]">
                    <Button type="submit" className="btn-blue btn-lg w-full flex items-center justify-center gap-4 py-3 !text-[14px] md:!text-[16px]">
                        {isLoading ? <CircularProgress size={24} color="inherit" /> : (
                            <>
                                <FaCloudUploadAlt className="text-[25px] text-white" />
                                <span className="whitespace-nowrap uppercase tracking-wide">Update Banner</span>
                            </>
                        )}
                    </Button>
                </div>
